...
public class GameOthello {
    ...
    private GameSquare otheloTable[][] = new GameSquare[8][8];
    ...
    public GameOthello(int x, int y, int w, int h, int depth) {
        int X = x;
        for (int j = 0; j < 8; j++) {
            for (int i = 0; i < 8; i++) {
                otheloTable[j][i] = new GameSquare(x, y, w, h, i, j);
                x = x + w;
            }
            y = y + h;
            x = X;
        ...
        lastMove = new GameMoves();
        setRules('B');
        this.depth = depth;
        r.gc();
    }

    public GameOthello(GameOthello oth) {

        Runtime r = Runtime.getRuntime();
        for (int j = 0; j < 8; j++) {
            for (int i = 0; i < 8; i++) {
                Vector<Point> v = new Vector<Point>();
                this.otheloTable[j][i] = new GameSquare(oth.getSquare(j, i).getX(), oth.getSquare(j, i).getY(), oth.getSquare(j, i).getW(), oth.getSquare(j, i).getH(), i, j);
        ...
        this.lastMove = new GameMoves(oth.getLastMove().getX(), oth.getLastMove().getY(), oth.getLastMove().getValue());
        r.gc();
    }

    public GameSquare getSquare(int j, int i) {
...
//-----------------------------------------------------------------------------------------------------------------------
    ...
    public ArrayList<GameOthello> getChildren(char player) {


        ArrayList<GameOthello> children = new ArrayList<GameOthello>();
        for (int y = 0; y < 8; y++) {
            for (int x = 0; x < 8; x++) {
                if (this.otheloTable[y][x].getAdd_circle()) {
                    GameOthello child = new GameOthello(this);
                    child.makeMove(y, x, player);
                    child.lastMove = new GameMoves(y, x);
                    if (player == 'B') {
                        child.setRules('W');
                    } else {
                        child.setRules('B');
    ...
    public void setPlayer() {
        ...
        player = now;
        if (now == 'W') {
            GameState state1 = new GameState(depth);
            GameMoves m = state1.alpha_beta(this);
            setRules('W');
